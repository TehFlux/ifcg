...
def createBrushSheet(baseName, sources, width, height, cellWidth, 
    cellHeight, padding, styles):
    """Create an SVG brush sheet from polygon sets.
    
    This function fills a brush sheet from the list of source polygon set. 
    Each polygon set is used to fill one cell."""
    # Rescale all sources to the cell size.
    rescaledSources = []
    numSources = len(sources)
    for s in sources:
        ps0 = cg.Polygon3Set()
        rescalePolys(s, ps0, cellWidth, cellHeight, padding)
        rescaledSources += [ps0]
    svgSheetFileName = ('temp' + os.path.sep + 'brushes' + os.path.sep 
        + baseName + ".svg")
    numCellsX = int(width / cellWidth)
    ...
    for i in xrange(0, numCellsX):
        for j in xrange(0, numCellsY):
            ps0 = rescaledSources[(j * numCellsY + i) % numSources]
            mt = cg.Matrix4.translate(
                -0.5 * width + 0.5 * cellWidth + i * cellWidth, 
                0.5 * height - 0.5 * cellHeight - j * cellHeight, 0.)
            ps0t = cg.Polygon3Set(ps0)
...
def scatterPolysMulti(source, numScattered, numTargetElements, 
    width, height, rScale, dScale, stdDevX, stdDevY, maxR):
    """Create multiple scatterings of polygons.
    
    Uses scatterPolys() to create multiple scattered sets of polygons from 
    the source, which are returned as a list."""
    print "  Creating scattered polygon sets..."
    result = []
    for i in xrange(0, numScattered):
        ps0 = cg.Polygon3Set()
        scatterPolys(source, ps0, numTargetElements, width, height, 
            rScale, dScale, stdDevX, stdDevY, maxR)
        result += [ps0]
    return result
...
def createRandomScatterBrush(baseName, brushName, sourceFileName, 
    sourceBaseName, numSourceShapes, numTargetShapes, numScattered, width, 
    height, cellWidth, cellHeight, padding, rScale, dScale, stdDevX, stdDevY, 
    maxR, styles):
    ...
    sources = scatterPolysMulti(ps0, numScattered, numTargetShapes, 
        width, height, rScale, dScale, stdDevX, stdDevY, maxR)
    createBrushSheet(baseName, sources, width, height, cellWidth, 
        cellHeight, padding, styles)
    ...
    createGIH(baseName, brushName, 
        ...
        width, height, cellWidth, cellHeight, "random")

sourceFileName = ('test' + os.path.sep + 'images' + os.path.sep 
    + 'shapes01.svg')
im.initRandom(4)
createRandomScatterBrush('RScatter01', 'T11 R Scatter 01', sourceFileName, 
    'shape', 5, 100, 100, 1000, 1000, 100, 100, 0.01, 
    cg.Range(10., 50.), cg.Range(0.1, 1.), 1., 1., 1., styles)
im.initRandom(2)
createRandomScatterBrush('RScatter02', 'T12 R Scatter 02', sourceFileName, 
    'shape', 5, 10, 100, 1000, 1000, 100, 100, 0.01, 
    cg.Range(20., 80.), cg.Range(0.3, 1.), 1., 1., 1., styles)
im.initRandom(5)
createRandomScatterBrush('RScatter03', 'T13 R Scatter 03', sourceFileName, 
    'shape', 5, 60, 100, 1000, 1000, 100, 100, 0.01, 
    cg.Range(20., 150.), cg.Range(0.05, 1.), 1., 1., 1., styles)
im.initRandom(12)
createRandomScatterBrush('RScatter04', 'T14 R Scatter 04', sourceFileName, 
    'shape', 5, 200, 100, 1000, 1000, 100, 100, 0.01, 
    cg.Range(10., 20.), cg.Range(0.05, 1.), 1., 1., 1., styles)
im.initRandom(72)
createRandomScatterBrush('RScatter05', 'T15 R Scatter 05', sourceFileName, 
    'shape', 5, 5, 100, 1000, 1000, 100, 100, 0.01, 
    cg.Range(80., 200.), cg.Range(0.8, 1.), 1., 1., 1., styles)

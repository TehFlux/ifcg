...
void Mesh::merge(Ionflux::GeoUtils::Mesh& other)
{
	Mesh* source = 0;
	bool ownSource = false;
	if (other.checkTransform())
	{
	    // create a copy with transformations applied
	    source = other.copy();
	    addLocalRef(source);
	    source->applyTransform(true);
	    ownSource = true;
	} else
	    source = &other;
	unsigned int numVerts0 = getNumVertices();
	unsigned int numVerts1 = source->getNumVertices();
	// vertices
	for (unsigned int i = 0; i < numVerts1; i++)
	{
	    Vertex3* cv0 = Ionflux::ObjectBase::nullPointerCheck(
	        source->getVertex(i), this, "merge", "Source mesh vertex");
	    addVertex(cv0->copy());
	}
	// faces
	unsigned int numFaces1 = source->getNumFaces();
	for (unsigned int i = 0; i < numFaces1; i++)
	{
	    Face* cf0 = Ionflux::ObjectBase::nullPointerCheck(
	        source->getFace(i), this, "merge", "Source mesh face");
	    Face* nf0 = cf0->copy();
	    nf0->applyVertexIndexOffset(numVerts0);
	    addFace(nf0);
	}
	// edges
	unsigned int numEdges1 = source->getNumEdges();
	for (unsigned int i = 0; i < numEdges1; i++)
	{
	    NFace* ce0 = Ionflux::ObjectBase::nullPointerCheck(
	        source->getEdge(i), this, "merge", "Source mesh edge");
	    NFace* ne0 = ce0->copy();
